...
class CheckoutController {
  ...
  Future<Either<Failure, CheckoutModel>> closeCheckout(
    ...
    var lastDate = DateFormat("yyyy-MM-dd").parse(checkoutProvider.getCheckoutModel()!.startedDate!).add(const Duration(days: 1)) ;
    var newDate = DateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'")
        .format(lastDate.subtract(const Duration(seconds: 1))) ;
    try{
      var response = await http.post(Uri.parse(CLOSE_CHECKOUT_URL),
          body: jsonEncode({
            "id": checkoutId,
            "closed_date": checkoutProvider.getActiveCheckout() == CheckoutStatus.passed ? newDate:closingDate,
            "price": price
          }),
          headers: headers);
